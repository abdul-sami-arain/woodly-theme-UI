 ...
 * @since    1.0
*/
if ( ! function_exists( 'woodly_breadcrumbs' ) ) {
    function woodly_breadcrumbs() {
        global $wp_query, $post, $paged;
        $space      = ' ';
        $on_front   = get_option( 'show_on_front' );
        $blog_page  = get_option( 'page_for_posts' );
        $separator  = $space . '<span class="bredcrumb-separator"><i class="ticon-chevron-right"></i></span>' . $space;
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : __( 'Our Blog', 'woodly' );
            $out .= sprintf( $link, home_url(), __( 'Home', 'woodly' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), __( 'Home', 'woodly' ) );
        }
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . __( '<span>Category</span> <span class="bredcrumb-separator"><i class="ticon-chevron-right"></i></span> ', 'woodly' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
               $out .= $separator . __( '<span>Tag</span> <span class="bredcrumb-separator"><i class="ticon-chevron-right"></i></span> ', 'woodly' ) . sprintf( $current, $wp_query->queried_object->name );
               
            } else if ( is_author() ) {
               
                $out .= $separator . __( '<span>Author</span>', 'woodly' );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                    $out .=  __( '<span>Day</span>','woodly');
                ...
                $out .= $separator . sprintf( $current, __( 'Error 404', 'woodly' ) );
            } elseif ( is_search() ) {
			if (have_posts()) {
        ...
        $out .= '</div>';
        echo apply_filters( 'woodly_breadcrumbs_out', $out );
    }
}
// Hooks into the Digital Store theme
add_action( 'woodly_before_template_header', 'woodly_breadcrumbs' );