...
if( !function_exists('woodly_wc_get_gallery_image_html') ) {
  // Copied and modified from woocommerce plugin and wc_get_gallery_image_html helper function.
  function woodly_wc_get_gallery_image_html( $attachment_id, $main_image = false, $size = 'woocommerce_single' ) {
...
add_action('pivoo_woo_sale_hook', 'pivoo_woo_sale_functions', 7);
  ...
  function woodly_woo_inventory_functions() {
      global $post, $product;
      if ( $product->get_stock_quantity() ) { // if manage stock is enabled 
if ( number_format($product->get_stock_quantity(),0,'','') < 10 ) { // if stock is low
echo '<span class="woodly-remaining-stock red-notice">Only ' . number_format($product->get_stock_quantity(),0,'','') . ' in stock</span>';
} else {
echo '<span class="woodly-remaining-stock green-notice">' . number_format($product->get_stock_quantity(),0,'','') . ' in stock</span>';
		}
	}
  }
add_action('woodly_woo_inventory_hook', 'woodly_woo_inventory_functions');
...
function pivoo_breadcrumb_delimiter( $defaults ) {
 ...
 if( class_exists( 'YITH_WCWL' ) ){
	 ...
	 */
	function woodly_product_wishlist_button() {
		?>
...
add_action( 'woocommerce_after_add_to_cart_button', 'woodly_product_wishlist_button',10, 0 );
