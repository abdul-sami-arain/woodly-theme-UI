...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				...
				'merlin_url'           => 'woodly-wizard',
				'parent_slug'          => 'woodly-admin-menu',
	...
	public function admin_page() {
		...
		wp_add_inline_style( 'merlin', '#qm-icon-container,#query-monitor-main{display:none}' ); // Hide Query Monitor plugin.
		 ...
		 wp_enqueue_style( 'merlin-dashboard-style',WOODLY_URL . '/assets/css/merlin-dashboard.min.css' );
		 
		 wp_enqueue_style( 'jquery-confirm-css',WOODLY_URL . '/assets/css/jquery-confirm.min.css' );
		...
		wp_enqueue_script('jquery-confirm',WOODLY_URL . '/assets/js/jquery-confirm.min.js', array('jquery'), '5.0', true);
		
		wp_enqueue_script('jquery-custom-select',WOODLY_URL . '/assets/js/jquery.customselect.min.js', array('jquery'), '5.0', true);
...
/**
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'woodly' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition merlin__content--transition-full-wdth">
		    ...
		    <?php if (class_exists( 'Woodly_Core' ) &&  class_exists( 'woocommerce' )) { ?>
			...
			<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
			<?php } else { ?>
			<h4 class="dh-plugin-required">Please Install and Activate Woodly Core & WooCommerce Plugin to continue import demo.</h4>
			<?php } ?>
...
/**
  ...
  /**
	...
	function elementor_update_options() {
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'product_template', 'footer_builder', 'edd_archive_builder', 'woodly_block' ] );
	}
